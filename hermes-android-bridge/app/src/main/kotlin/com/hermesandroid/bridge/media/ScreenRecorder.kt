...
object ScreenRecorder {
    ...
    /** Non-blocking start: stores recording state for later stop(). */
    fun startRecording(): Map<String, Any?> {
        ...
        synchronized(this) {
            if (recorder != null) return mapOf("success" to false, "message" to "Already recording")
        }
        ...
        handler.post {
            ...
            try {
                ...
                projection.registerCallback(object : MediaProjection.Callback() {
                    override fun onStop() { cleanupRecording() }
                }, handler)
                ...
                mr.start()
                resultHolder[0] = mapOf("success" to true, "message" to "Recording started")
            } catch (e: Exception) {
                cleanupRecording()
                projection?.stop()
                if (e is SecurityException || e is IllegalStateException) clearProjectionPermission()
                resultHolder[0] = mapOf("success" to false, "message" to "Start failed: ${e.message}")
            } finally {
                latch.countDown()
            }
        }
        latch.await(5000, java.util.concurrent.TimeUnit.MILLISECONDS)
        return resultHolder[0] ?: mapOf("success" to false, "message" to "Start timed out")
    }

    /** Non-blocking stop: stops recording and returns video data. */
    fun stopRecording(): Map<String, Any?> {
        ...
        handler.post {
            try {
                val mr = recorder ?: run {
                    resultHolder[0] = mapOf("success" to false, "message" to "Not recording")
                    latch.countDown()
                    return@post
                }
                ...
                if (outputFile != null && outputFile.exists()) {
                    ...
                    resultHolder[0] = mapOf(
                        "success" to true, "message" to "Recording stopped",
                        "data" to mapOf(
                    ...
                    resultHolder[0] = mapOf("success" to true, "message" to "Recording stopped (no output)")
                }
            } catch (e: Exception) {
                cleanupRecording()
                resultHolder[0] = mapOf("success" to false, "message" to "Stop failed: ${e.message}")
            } finally {
                latch.countDown()
            }
        }
        latch.await(10000, java.util.concurrent.TimeUnit.MILLISECONDS)
        return resultHolder[0] ?: mapOf("success" to false, "message" to "Stop timed out")
    }
